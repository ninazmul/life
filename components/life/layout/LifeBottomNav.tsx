...
export function LifeBottomNav({ onOpenMore, activeLoansCount = 0 }: LifeBottomNavProps) {
  ...
  return (
    <nav className="md:hidden fixed bottom-0 left-0 right-0 z-40 bg-slate-950/90 dark:bg-[#070a12]/95 backdrop-blur-xl border-t border-slate-800/80 pb-safe">
      <div className="flex items-center justify-around h-16 px-2">
            ...
            >
              <div className="relative">
                ...
                {item.badge !== undefined && (
                  <span className="absolute -top-1.5 -right-2.5 flex h-4 min-w-4 px-1 items-center justify-center rounded-full bg-amber-500 text-slate-950 text-[10px] font-extrabold">
                    {item.badge}
                  </span>
                )}
              </div>
        ...
        >
          <Grid
            className={`w-5 h-5 transition-transform ${
              isMoreActive ? "scale-110 stroke-[2.5]" : "stroke-[1.8]"
            }`}
          />
